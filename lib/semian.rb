...
if Semian.supported_platform? && !Semian.enabled?
  ...
  Semian.logger.info("Semian is not supported on #{RUBY_PLATFORM} - all operations will no-op") if !Semian.supported_platform?
  Semian.logger.info("Semian is disabled, is this what you really want? - all operations will no-op") if !Semian.enabled?
end
